...
#         laptop = {'Brand' : 'Acer' ,'Link' : links[i], 'Name' : names[i], 'Sale Type' : sale_types[i],'Old Price' : old_prices[i],'Price' : prices[i], 'Processor' : processors[i], 'GPU' : gpus[i], 'RAM' : rams[i], 'Storage' : storages[i]}
...
def merge_lists(links, names, prices, processors, rams, gpus, storages, old_prices, sale_types):
    ...
    for i in range(len(links)):
        laptop = {
            ...
            'link': links[i],
            'name': names[i],
            'originalPrice': old_prices[i],
            'saleType': sale_types[i],
            'finalPrice': prices[i],
...
with open("AcerHTML(106).json", "w") as f:
