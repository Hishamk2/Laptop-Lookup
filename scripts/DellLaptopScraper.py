...
def get_total_num_pages(total_num_results, num_results_per_page):
    return ceil(int(total_num_results) / int(num_results_per_page))
...
#         laptop = {'Brand' : 'Dell' ,'Link' : links[i], 'Name' : names[i], 'Original Price' : sale[i], 'Sale Type' : sale_type[i], 'Price' : prices[i], 'Processor' : processors[i], 'GPU:' : GPUs[i], 'RAM' : rams[i], 'Storage' : storage[i]}
...
def merge_lists(sale_type, names, prices, processors, rams, links, GPUs, storage, sale, all_laptops):
    ...
    for i in range(len(names)):
        laptop = {
            ...
            'link': links[i],
            'name': names[i],
            'originalPrice': sale[i],
            'saleType': sale_type[i],
            'finalPrice': prices[i],
...
def get_laptop_link(soup):
    ...
    for link in laptop_link_html:
        ...
        if x != None:
            ...
            # Remove the first 2 characters from the link as it starts with '//' and when you click on the link it doesn't work properly (goes to file:// then link)
...
# """
i = 1
URL = f'https://www.dell.com/en-ca/search/laptop?p={i}&t=Product'
...
for i in range(1, total_num_pages + 1):
    # random number between 100 and 1500
    random_time = random.randint(100, 1500)
    ...
    with open(f'DellHTMLFinal({i}).json', "w") as f:
        json.dump(all_laptops, f, indent=4)
